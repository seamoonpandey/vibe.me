...
class SmartListTest {
    private fun s(id: Long, added: Long) = me.vibe.data.Song(
    ...
    fun `recently added is newest first`() {
        val out = me.vibe.data.smartListSongs(
            me.vibe.data.SmartList.RECENTLY_ADDED, songs, emptyMap(), emptyMap(),
        )
    ...
    fun `most played excludes never-played tracks`() {
        val out = me.vibe.data.smartListSongs(
            me.vibe.data.SmartList.MOST_PLAYED, songs, mapOf(1L to 2, 3L to 9), emptyMap(),
        )
    ...
    fun `recently played is ordered by last play, not by count`() {
        val out = me.vibe.data.smartListSongs(
            me.vibe.data.SmartList.RECENTLY_PLAYED, songs,
...
class GreetingTest {
    private fun g(h: Int) = me.vibe.ui.greetingFor(h, "moon")
    ...
    fun `no name means no dangling comma`() {
        assertEquals("Good morning", me.vibe.ui.greetingFor(9, ""))
        assertEquals("Good morning", me.vibe.ui.greetingFor(9, "   "))
    }

    @Test
    fun `the prompt is stable for an hour and always in range`() {
        assertEquals(me.vibe.ui.promptFor(9, 200), me.vibe.ui.promptFor(9, 200))
        for (h in 0..23) for (d in 1..366) me.vibe.ui.promptFor(h, d)
    }
}
